...
import { JWTClaims, JWTToken, RefreshToken, User } from "../../domain";
...
export class RedisAuthService extends AbstractRedisClient
  ...
  public signJWT(props: JWTClaims): JWTToken {
    return jwt.sign(props, authConfig.secret as string, {
      expiresIn: authConfig.tokenExpiredTime
    });
  }
  ...
  public decodeJWT(token: string): Promise<JWTClaims> {
    return new Promise((resolve, reject) => {
      jwt.verify(token, authConfig.secret as string, (err, decoded) => {
        if (err) return resolve(err as any);
